...
function myc_detect_intent( $request ) {
    ...
    $session = $sessions_client->sessionName( $project_id, $session_id );

    $query_input = new QueryInput();
    $optionals_params = array();
    ...
    $response = $sessions_client->detectIntent( $session, $query_input, $optionals_params );
    ...
    while ($fulfillment_messages[$i]) {
        ...
        // oEmbed support
        if ( isset( $current_message['text'] ) ) {
            remove_filter( 'the_content', 'wpautop' );
            $current_message['text']['text'][0] = apply_filters( 'the_content', $current_message['text']['text'][0] );
        }
